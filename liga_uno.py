...
def formatear_valor(valor, moneda):
    ...
    if valor >= 1_000_000:
        return f"{valor / 1e6:.2f} M{simbolo}"
    if valor >= 1_000:
        return f"{valor / 1e3:.0f} K{simbolo}"
    return f"{valor:,.0f} {simbolo}"
...
def actualizar() -> None:
    ...
    df = df[df["posicion"].map({v: k for k, v in POS_ESP.items()}).isin(FILTRO_POSICIONES)]
...
if __name__ == "__main__":
    ...
    if os.path.exists(OUT) and not forzar:
        ...
        print("Corré 'python liga_uno.py --actualizar' para actualizarlos.")
    else:
        actualizar()
