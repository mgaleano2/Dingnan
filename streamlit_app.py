...
st.set_page_config(page_title="China League 1 · Top Jugadores", layout="wide", initial_sidebar_state="expanded")
...
# ======================
with st.sidebar:
    st.markdown("## China League 1")
    st.caption("Temporada 2026 · Sofascore")
    st.divider()
    ...
    paises = st.multiselect("País", sorted(df["País"].dropna().unique()), default=[])
    pie = st.multiselect("Pie", sorted(df["Pie"].dropna().unique()), default=[])
...
with tab_tabla:
    ...
    st.dataframe(vista, use_container_width=True, height=520, column_config=config)

    csv = vista.to_csv(index=False).encode("utf-8-sig")
    xlsx = vista.to_excel(index=False) if False else None
